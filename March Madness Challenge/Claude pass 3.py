...
def calculate_strength(team_name, round_factor=1.0, home_court_advantage=0):
    ...
    x_factor = random.uniform(-1.5, 1.5)  # Using a uniform distribution for X-factor
    momentum = team['momentum']

    strength = off_def_balance * 0.4 + shooting_efficiency * 0.2 + rebounding_factor * 0.15 + turnover_factor * 0.15 + seed_factor * 0.15 + offensive_efficiency_factor * 0.03 + x_factor + momentum + home_court_advantage
    return strength
...
def simulate_game(team1, team2, round_factor=1.0, home_court_advantage=0):
    ...
    if strength1 > strength2:
        ...
        team_stats[team1]['momentum'] += margin / 200  # Adjust momentum based on performance, but with a smaller impact
        return team1
    else:
...
def simulate_round(matchups, round_factor=1.0, home_teams=None):
    ...
    for i, matchup in enumerate(matchups):
        team1, team2 = matchup
        home_court_advantage = 1.5 if home_teams and team1 in home_teams else 0  # Reduced home-court advantage
        winner = simulate_game(team1, team2, round_factor=0.7, home_court_advantage=home_court_advantage)
...
def run_tournament_simulation(num_simulations=100000):
    ...
    for _ in range(num_simulations):
        ...
        elite_eight_winners = simulate_round(sweet_16_matchups, 0.7)
        final_four_winners = simulate_round([(elite_eight_winners[i], elite_eight_winners[i + 1]) for i in range(0, len(elite_eight_winners), 2)], 1.2)
        championship_game = [(final_four_winners[0], final_four_winners[1])]
        home_team = random.choice(championship_game)  # Randomly assigning home-court advantage
        champion = simulate_round(championship_game, 1.5, [home_team])[0]
